...
class MainWindow(QMainWindow):
    def __init__(self, store: ConfigStore, icon_path: str) -> None:
        ...
        if not self.backend.start():
            if self.backend.available:
                QTimer.singleShot(0, self._show_mpv_start_failed)
            else:
    ...
    def retranslate_ui(self) -> None:
        ...
        self.playlists_heading.setText("EVIL PLAYLISTS")
    ...
    def _playlist_display_name(self, name: str) -> str:
        """Return the localized label for system playlists without renaming stored keys."""
        if name == FAVORITES_PLAYLIST:
            return tr(self.language, "favorites")
        return name

    def _refresh_playlist_sidebar(self) -> None:
        self.playlist_list.clear()
        for name, tracks in self.playlists.items():
            display_name = self._playlist_display_name(name)
    ...
    def _update_playlist_summary(self) -> None:
        if not hasattr(self, "current_playlist_label"):
            return
        self.current_playlist_label.setText(self._playlist_display_name(self.current_playlist))
        ...
        self._update_hero_cover()

    @staticmethod
    def _square_cover_pixmap(pixmap: QPixmap, size: int) -> QPixmap:
        if pixmap.isNull():
            return QPixmap()
    ...
    def _embedded_cover_data(self, path: str) -> bytes | None:
        if path in self._cover_cache:
            return self._cover_cache[path]

        # Playlists may contain tracks that were moved, renamed or deleted.
        # A missing audio file must never prevent Evil Spotify from starting.
        if not Path(path).is_file():
            self._cover_cache[path] = None
            return None
        ...
        try:
            ...
            if audio is not None:
                # FLAC and formats exposing a native pictures collection.
                ...
                if cover_data is None and tags:
                    # MP3 / ID3 APIC frames.
                    ...
                    if cover_data is None:
                        ...
                        if covers:
                            ...
                            try:
                                cover_data = bytes(first)
                            except (TypeError, ValueError):
                                pass

                    # Ogg/Opus often stores a base64 FLAC picture block.
                    if cover_data is None:
                        try:
                            picture_blocks = tags.get("metadata_block_picture", [])
                        except (AttributeError, TypeError):
                            picture_blocks = []
                        if picture_blocks:
                            if not isinstance(picture_blocks, (list, tuple)):
                                picture_blocks = [picture_blocks]
                            for encoded in picture_blocks:
                                try:
                                    raw = base64.b64decode(encoded if isinstance(encoded, (bytes, bytearray)) else str(encoded).encode("ascii"))
                                    picture = Picture(raw)
                                    if picture.data:
                                        cover_data = bytes(picture.data)
                                        break
                                except (ValueError, TypeError):
                                    continue

                    # Some Vorbis files use a plain base64 coverart field.
                    if cover_data is None:
                        try:
                            ...
                            coverart = []
                        if coverart:
                            if isinstance(coverart, (list, tuple)):
                                coverart = coverart[0]
                            try:
                                cover_data = base64.b64decode(coverart if isinstance(coverart, (bytes, bytearray)) else str(coverart).encode("ascii"))
                            except (ValueError, TypeError):
                                pass

                    # APEv2 convention: "Cover Art (Front)" contains a filename
                    # terminated by NUL, followed by the raw image bytes.
                    if cover_data is None and hasattr(tags, "keys"):
                        for key in tags.keys():
                            if not str(key).lower().startswith("cover art"):
                                continue
                            try:
                                raw_value = bytes(tags[key])
                            except (TypeError, ValueError):
                                continue
                            if b"\x00" in raw_value:
                                raw_value = raw_value.split(b"\x00", 1)[1]
                            if raw_value:
                                cover_data = raw_value
                                break
        except (MutagenError, OSError, ValueError, TypeError, AttributeError):
            cover_data = None

        # Validate that Qt can actually decode the bytes before caching them.
        if cover_data:
            test_pixmap = QPixmap()
            if not test_pixmap.loadFromData(cover_data):
                cover_data = None

        self._cover_cache[path] = cover_data
        return cover_data

    def _cover_pixmap(self, path: str, size: int) -> QPixmap:
        data = self._embedded_cover_data(path)
        if data:
            pixmap = QPixmap()
            if pixmap.loadFromData(data):
                return self._square_cover_pixmap(pixmap, size)
        return self.icon_pixmap.scaled(
            size,
            size,
            Qt.AspectRatioMode.KeepAspectRatio,
            Qt.TransformationMode.SmoothTransformation,
    ...
    def _set_player_cover(self, path: str | None = None) -> None:
        if not hasattr(self, "player_icon"):
            return
        self.player_icon.setPixmap(self._cover_pixmap(path, 54) if path else self.icon_pixmap.scaled(
            54, 54, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation
        ))

    def _update_hero_cover(self, preferred_path: str | None = None) -> None:
        if not hasattr(self, "hero_icon"):
            return
        tracks = self.playlists.get(self.current_playlist, [])
        candidates: list[str] = []
        if preferred_path and preferred_path in tracks:
            candidates.append(preferred_path)
        if self.current_path and self.current_path in tracks and self.current_path not in candidates:
            candidates.append(self.current_path)
        candidates.extend(path for path in tracks if path not in candidates)

        for path in candidates:
            if self._embedded_cover_data(path):
                self.hero_icon.setPixmap(self._cover_pixmap(path, 138))
                return
        self.hero_icon.setPixmap(self.icon_pixmap.scaled(
            138, 138, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation
        ))
    ...
    def _track_metadata(self, path: str) -> tuple[str, str, str, str]:
        ...
        # Keep stale playlist entries visible (with their filename) instead of
        # crashing while trying to read metadata from a file that no longer exists.
        if not Path(path).is_file():
    ...
    def _play_index(self, index: int, record_shuffle_history: bool = True) -> None:
        ...
        if self.backend.load(path):
            ...
            self._set_player_cover(path)
            self._update_hero_cover(path)
            self.play_button.setIcon(
    ...
    def _toggle_frequency(self, checked: bool) -> None:
        ...
        self.store.save_settings()
        success = self._apply_audio_filters()
        if checked and success:
            if self.backend.last_pitch_method == "rubberband-r3":
                message_key = "frequency_on_r3"
            elif self.backend.last_pitch_method == "scaletempo-compat":
                message_key = "frequency_on_compat"
            else:
                message_key = "frequency_on"
        else:
            message_key = "frequency_on" if checked else "frequency_off"
        self.statusBar().showMessage(tr(self.language, message_key), 3500)
    ...
    def _apply_audio_filters(self) -> bool:
        if not self.backend.available:
            return False
    ...
    def _handle_mpv_event(self, event: dict) -> None:
        ...
        if event_name == "property-change":
            ...
            # Re-apply the explicit pitch/EQ chain only after mpv has a real
            # audio stream. This avoids false-success filter setup before the
            # decoder/audio chain is fully configured.
            QTimer.singleShot(0, self._apply_audio_filters)
    ...
    def _show_mpv_start_failed(self) -> None:
        detail = self.backend.last_start_error.strip()
        message = tr(self.language, "mpv_start_failed")
        if detail:
            message = f"{message}\n\n{detail}"
        QMessageBox.warning(
            self,
            tr(self.language, "mpv_start_failed_title"),
            message,
        )
